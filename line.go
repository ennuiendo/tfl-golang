...
func (c *Client) GetLineArrivals(ctx context.Context, lines []string, stopid string, params map[string]string) ([]Prediction, error) {
	path := fmt.Sprintf("/Line/%s/Arrivals/%s", conv.StringSliceToString(lines), stopid)

	predictions := []Prediction{}
	err := c.getWithQueryParams(ctx, path, params, &predictions)
	if err != nil {
		return nil, err
	}
